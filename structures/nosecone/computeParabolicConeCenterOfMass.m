function CoM = computeParabolicConeCenterOfMass(length, thickness, k)
% 
% Matt Werner (m.werner@vt.edu) - Jan 14, 2021
% 
% Reference the distance of a parabolic cone's center of mass with respect
% to its tip. The cone may be completely solid or completely hollow,
% consisting only of a shell of material on its outside. Determining the
% cone's hollowness is determined by the thickness, where 'length'
% thickness corresponds with a solid cone and 0 thickness corresponds with
% the shell.
% 
...
%    Outputs:
% 
%               CoM - The distance between the cone's vertex and the cone's
%                     center of mass.
%                     Size: 1-by-1 (scalar)
%                     Units: m (meters)
% 
...
% Define a nondimensional parameter
l = thickness/length;

% Precompute repeated powers of l and k
l2 = l^2;
l3 = l2*l;
l4 = l3*l;
l5 = l4*l;
...
% dependent upon the thickness (l) and curvature (k)
num = 10*(l3 - 6*l + 8) + 6*(l4 - 10*l2 + 20*l - 15)*k + (l5 - 15*l3 + 40*l2 - 45*l + 24)*k2;
den = 20*(l2 - 3*l + 3) + 15*(l3 - 4*l2 + 6*l - 4)*k + 3*(l4 -5*l3 + 10*l2 - 10*l + 5)*k2;
correctionFactor = num/den;

% Center of mass of a constant density cylinder (solid or hollow)
CoMCylinder = length/2;

% Calculate the distance of a parabolic cone's center of mass from its tip
% (result is independent of the base radius) as the perturbation away from
% the center of mass of a solid cylinder
CoM = CoMCylinder*correctionFactor;