function [n, m, gnm, hnm, dgnmdt, dhnmdt] = loadMagneticCoefficients(N, M)
% 
% Matt Werner (m.werner@vt.edu) - Dec 2, 2020
% 
% Load the magnetic coefficients defined by the World Magnetic Model of
% 2020 (WMM2020). Although the magnetic field model is still a spherical
% harmonic expansion of the magnetic potential, its maximum degree and
% order is much, much smaller than that of the gravitational potential
% model (EGM2008), topping out at a mere (but full) degree and order 12.
% The epoch for all magnetic field coefficients is taken to be 2020.0, 
% that is, Jan 1, 2020 00:00:000 UTC. Again, unlike the gravitational 
% potential, all magnetic field coefficients come with a secular rate of 
% change such that the actual coefficients be evaluated at a time 
% t > 2020.0 according to the simple linear variation.
% 
%    Inputs:
% 
%                 N - Maximum desired degree of the magnetic model. WMM2020
%                     provides a maximum degree of 12.
%                     Size: 1-by-1 (scalar)
%                     Units: - (unitless)
% 
%                 M - Maximum desired order of the magnetic model. WMM2020
%                     provides a maximum order of 12.
%                     Size: 1-by-1 (scalar)
%                     Units: - (unitless)
% 
%    Outputs:
...
%                     gnm, hnm, dgnm/dt, and dhnm/dt are evaluated.
%                     Size: L-by-1 (vector)*
%                     Units: - (unitless)
% 
%                 m - List of degrees at which each harmonic coefficient
%                     gnm, hnm, dgnm/dt, and dhnm/dt are evaluated.
%                     Size: L-by-1 (vector)*
%                     Units: - (unitless)
% 
...
disp("Loading magnetic model...")
% Check if degree is specified too high
if (N > 12)
    error("Cannot supply degrees N exceeding 12.")
end
...
if (M > 12)
    warning("Changing order M to maximum order (12).")
    M = 12;
end
...
% Discard undesired degrees and orders
MagMat(MagMat(:, 1) > N, :) = [];
MagMat(MagMat(:, 2) > M, :) = [];

% Distribute 
[n, m] = deal(MagMat(:,1), MagMat(:,2));
[gnm, hnm] = deal(MagMat(:,3), MagMat(:,4));
[dgnmdt, dhnmdt] = deal(MagMat(:,5), MagMat(:,6));

% Display
checkCoefficients(gnm, hnm, "Magnetic")
if (N == M)
    disp("Magnetic model is full degree and order (degree " + N + " and order " +  M + ")")
end
